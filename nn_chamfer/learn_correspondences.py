...
def repulsion_loss(points, radius=0.02):
    """
    Repulsion loss that penalizes predicted points for being too close to each other.
    This prevents point cloud collapse by encouraging points to spread out.
    
    Args:
        points: (N, 3) tensor of predicted points
        radius: Radius parameter controlling the repulsion strength (default: 0.02)
    
    Returns:
        repulsion_loss: Scalar loss value
    """
    if points.shape[0] < 2:
        return torch.tensor(0.0, device=points.device)
    
    # Compute pairwise differences: (N, N, 3)
    diff = points.unsqueeze(0) - points.unsqueeze(1)  # (1, N, 3) - (N, 1, 3) = (N, N, 3)
    
    # Compute squared distances: (N, N)
    dist2 = (diff ** 2).sum(dim=-1)
    
    # Repulsion term: exp(-dist^2 / radius^2)
    # Large when points are close, small when points are far
    rep = torch.exp(-dist2 / (radius ** 2))
    
    # Exclude diagonal (self-distances) by masking
    mask = ~torch.eye(points.shape[0], dtype=torch.bool, device=points.device)
    rep_masked = rep[mask]
    
    # Return mean repulsion (higher = more collapse, so we want to minimize this)
    return rep_masked.mean()


def column_entropy_loss(probabilities):
    """
    Column-wise entropy regularization to prevent correspondence collapse.
    Encourages each target point Y[j] to be used evenly across all source points.
    
    This is the equivalent of repulsion in correspondence space - prevents all rows
    from picking the same column.
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                for i in range(len(X_batch)):
                    ...
                    # Compute correspondences using dot product
                    correspondances = model.compute_correspondances(X, Y)  # (N, M) correspondence matrix
                    
                    # Apply softmax to get probabilities
                    probabilities = model.softmax_correspondances(correspondances)  # (N, M)
                    
                    # Compute virtual point cloud: Y_hat = probabilities @ Y
                    ...
                    # Chamfer Distance Loss (bidirectional nearest neighbor)
                    # Forward: distance from each Y_hat point to nearest Y point
                    distances_forward = torch.cdist(Y_hat, Y)  # (N, M)
                    min_dist_forward = distances_forward.min(dim=1)[0]  # (N,)
                    
                    # Backward: distance from each Y point to nearest Y_hat point
                    distances_backward = torch.cdist(Y, Y_hat)  # (M, N)
                    min_dist_backward = distances_backward.min(dim=1)[0]  # (M,)
                ...
                with torch.no_grad():
                    for batch in val_loader:
                        ...
                        for i in range(len(X_batch)):
                            ...
                            # Chamfer Distance Loss (bidirectional)
                            distances_forward = torch.cdist(Y_hat, Y)
                            min_dist_forward = distances_forward.min(dim=1)[0]
                            distances_backward = torch.cdist(Y, Y_hat)
...
def compute_virtual_point_cloud(model, P, Q):
    ...
    # Compute correspondances using dot product
